...
2. **Build the correct URI.** Use the pattern:
   ...
   ```
   mongodb+srv://USERNAME:PASSWORD@CLUSTER.mongodb.net/ecatu_portfolio?retryWrites=true&w=majority&appName=AppName
   ```

   Example (replace values in UPPER-CASE):

   ```env
   MONGODB_URI=mongodb+srv://USERNAME:PASSWORD@CLUSTER.mongodb.net/ecatu_portfolio?retryWrites=true&w=majority&appName=Ecatu
   DATABASE_REQUIRED=true
   ```
...
5. **Start the server** and call the health endpoint:
   ...
   A successful response includes `"contactStorage": "mongodb"` and `"database"."state": "connected"`. If `contactStorage` is `"file"` or `"database"."usingFallbackStorage"` is `true`, MongoDB is not yet connected — review the steps above and check the error hints in the server log.
